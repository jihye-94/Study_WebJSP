...
<!--  
	...
	3. application(전역객체: web.xml 자원 접근 read , 전역변수 생성가능)
	...
	*session객체 (활용도 ^^)
    ...
    Life-cycle : 1. session변수는 session객체와 동일 
                 ...
                 로그아웃 버튼 클릭시 session.incaildate()
    ...
    *application 객체
    ...
    활용: 전체 접속자수, 현재 접속자수, 방문자수 카운팅 등
    
    *request 객체
    [내부에 변수 생성] : request.setAttribute("type","A");
    scope: 요청 페이지(login.jsp >> 요청당 request 생성 >> scope >> page
    요청한 페이지에서만 사용가능하지만 ^^^^^예외적으로 ^^^^^^^^^
    login.jsp >> ok.jsp(include or forward했다면) request 객체는 ok.jsp 공유된다           
-->
...
<%
	...
	//session변수 empobj는 어느 페이지에서나 사용 가능하다.
	//만약 Ex20_sample.jsp에서 사용하고싶다면 
	...
	b.jsp는 
	application, session 변수 모두 사용 가능
	session.setAttribute("empobj", emp); 만든 것 
	include, forward 사용 가능 
	*/
%>
</body>
</html>
